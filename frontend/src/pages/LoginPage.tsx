...
function LoginPage() {
    ...
    const congnitoSignIn = async () => {
        try {
            setFeching(true)
            const user = await AuthenticateUser(userName, password)
            setFeching(false)

            localStorage.setItem("user", user.cognitoUser.getUsername())
            localStorage.setItem("accessToken", user.session?.getIdToken().getJwtToken() as string)
                                ...
                                <div>
                                    <div className="mt-3 text-left sm:mt-5">
                                        <div className="inline-flex items-center w-full">
                                            <h3 className="text-lg font-bold text-neutral-600 l eading-6 lg:text-5xl">Signin</h3>
                                        </div>
                                        <div className="mt-4 text-base text-gray-500">
                                            <p>Signin and get our newest news.</p>
                                        </div>
                                    </div>
                                </div>
